...
public class PercentageSplitStrategy implements ExpenseStrategy{
    ...
    public PercentageSplitStrategy(UserController userController) {
        this.userController = userController;
    }

    @Override
    public void splitAmount(String userId, Expense expense) {
        double totalPercentage = expense.getSplitRatio().values().stream().mapToDouble(k->k).sum();
        if(totalPercentage>=100 || totalPercentage <0) throw  new IllegalArgumentException("percentage Split Expense incorrect");

        for(Map.Entry<String,Double> e: expense.getSplitRatio().entrySet()){
            double cursplitAmount = e.getValue()*expense.getAmount()/100;
